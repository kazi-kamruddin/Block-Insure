...
\hline
Document hash, CID, evidence chain, and Merkle root & Supports integrity
checking and auditability & IPFS is not private; off-chain chain and registry
can be rewritten by privileged operators \\
\hline
Two-oracle quorum and timeout & Requires two confirmations and fails split
decisions conservatively & Workers share infrastructure; root matching is not
an enforced verification gate \\
\hline
Reserve intelligence and warning & Exposes liabilities and emits low-reserve
warning & Withdrawal is not liability-constrained \\
\hline
\end{tabular}
}
\end{table}
...
and key rotation. In addition, the monolithic compiled contract has a deployed
runtime size of 30,028 bytes, which exceeds the 24,576-byte EIP-170 limit by
5,452 bytes. Local Hardhat execution enables
\texttt{allowUnlimitedContractSize}; deployment to an EIP-170-enforcing public
network requires contract modularization.
